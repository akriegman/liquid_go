...
export function formatScore(score) {
  if (cookie('number_style') == 'exponential')
  {
    return score == 0 ? '0' : (Math.sign(score) == -1 ? '-' : '') + 'e' + Math.log10(Math.abs(score)).toFixed(2);
  } else if (Math.abs(score) >= 10_000_000) {
...
export function mobileOrTablet() {
  let check = false;
