...
import { formatScore, mobileOrTablet, Option, Radio } from "./utils";
...
export default class Singleplayer extends Component {
  ...
  componentDidMount() {
    ...
    this.intervalID = setInterval(() => {
      this.setState({
        prisoners: this.board.current.spill(
          this.blackSample(),
          this.whiteSample(),
        ),
      });
  ...
  render() {
    ...
    return (
      <Board
        ref={this.board}
        cells={this.state.cells <= 0 ? 1 : Number(this.state.cells)}
        pixels={Number(this.state.pixels)}
        flow={this.state.flow}
        capturing={this.state.capturing}
      >
        <Radio
          value={this.state.blackStrat}
          on={(value) => this.setState({ blackStrat: value })}
        >
        ...
        <br />
        <Radio
          value={this.state.whiteStrat}
          on={(value) => this.setState({ whiteStrat: value })}
        >
        ...
        <label>
          ...
          <input
            ...
            onInput={(event) => this.setState({ pixels: event.target.value })}
          >
          </input>
        </label>
        <br className="big" />
        <label>
          ...
          <input
            ...
            onInput={(event) => this.setState({ cells: event.target.value })}
          >
          </input>
        </label>
        <br className="big" />
        <label>
          <input
            type="checkbox"
            defaultChecked={this.state.capturing}
            onInput={() => this.setState({ capturing: !this.state.capturing })}
          >
          </input>
          &nbsp;Enable capturing
        </label>
        ...
        <label>
          ...
          <input
            ...
            min={Math.floor(this.area() / 14400)}
            max={Math.floor(this.area() / 1200)}
            onInput={(event) =>
              this.setState({ flow: Number(event.target.value) })}
          >
          </input>
        </label>
        <br className="big" />
        <Radio
          ...
          on={(value) => this.setState({ scoreType: value })}
        >
        ...
        <button
          onClick={() => {
            this.setState({
              score: this.board.current.score(this.state.scoreType),
            });
          }}
        >
          Calculate score
  ...
  sampleFunc = (isBlack, strat) => {
    this.board.current?.board.set_speed(isBlack, strat == "follow" ? 0.25 : 1);
    return strat === "mouse left"
      ? () => ({
      ...
      : strat === "sine"
      ? () => ({
        x: (Math.sin(performance.now() / 300 + (isBlack ? 100 : 50)) + 1) *
          this.state.cells / 2,
        y: (Math.sin(
          performance.now() / 300 * (1 + Math.sqrt(5)) / 2 + (isBlack ? 20 : 0),
        ) + 1) * this.state.cells / 2,
        active: true,
      })
      : strat === "follow"
      ? () => ({
        x: this.board.current[isBlack ? "whiteMouse" : "blackMouse"].x,
             y: this.board.current[isBlack ? "whiteMouse" : "blackMouse"].y,
